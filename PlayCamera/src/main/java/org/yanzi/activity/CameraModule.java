...
import org.yanzi.example.ImageObject;
...
/**
 * Created by zhangcirui on 15/12/30.
 */
public class CameraModule {

    private static final String TAG = CameraModule.class.getSimpleName();
    private static CameraModule mInstantce;
    private File PHOTO_DIR = new File(Environment.getExternalStorageDirectory().getPath() + "/aigo_kt03/camera.jpg");

    public static CameraModule getInstance() {

        if (mInstantce == null) {
            mInstantce = new CameraModule();
        }
        return mInstantce;
    }
    ...
    public void uploadImage(File file, String uploadUrl,String uploadSign) {

        Log.d(TAG, file.getPath() + " : " + uploadUrl+" : "+uploadSign);
